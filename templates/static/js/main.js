...
var Cafe = {
  ...
  init: function(options) {
    ...
    Cafe.firstCategory = options.firstCategory;

    $('.cafe-item').addClass('hide').filter( '[data-category*="' + Cafe.firstCategory + '"]' ).removeClass( 'hide');
  ...
  toggleCategories: function(loading) {
    Cafe.isCategories = loading;
    $('section.cafe-categories').toggleClass('hide', !!Cafe.isCategories);
  },
  mainBtnClicked: function() {
    ...
    if (Cafe.modeOrder) {
      ...
      Cafe.apiRequest('makeOrder', params, function(result) {
        ...
        if (result.ok) {
          if (invoiceSupported) {
            Telegram.WebApp.openInvoice(result.result, function(status) {
              if (status == 'paid') {
...
$('a').click(function() {
  ...
  $('.cafe-item').addClass('hide').filter( '[data-category*="' + category + '"]' ).removeClass( 'hide');
